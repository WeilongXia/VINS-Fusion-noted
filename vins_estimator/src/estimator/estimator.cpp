...
{
    inputImageCnt++;
    // featureID, cameraID, (x, y, z, u, v, v_x, v_y)
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header)
{
    ...
    if (ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            ...
            std::cout << "calib_ric: " << calib_ric << std::endl;
        }
    }
    ...
    if (solver_flag == INITIAL)
    {
        ...
        if (!STEREO && USE_IMU)
        {
            ...
            {
                bool result = false;
                // 有外参并且当前帧时间戳大于初始化时间戳0.1s，就进行初始化操作
...
bool Estimator::initialStructure()
{
    ...
    // 计算all_image_frame中所有图像的IMU加速度的标准差
    ...
    {
        // it_per_id是FeaturePerID对象，start_frame代表特征点观测起始帧
        int imu_j = it_per_id.start_frame - 1;
        // tmp_feature保存了某个特征点在所有相机观测下的对应坐标
        SFMFeature tmp_feature;
        tmp_feature.state = false;
        tmp_feature.id = it_per_id.feature_id;
        // feature_per_frame包含了某个特征点的所有观测帧信息
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            // 为什么是imu_j++呢？如果某个特征点在第1帧看到了，第2帧没看到，但是第3帧又看到了，是否不合理？
            imu_j++;
    ...
    // 此处的relative_R, relative_T为当前帧到参考帧（第l帧）的坐标变换R、t
    ...
    // 对于所有的图像帧，包括关键帧和非关键帧，提供初始的RT估计，然后solvePnP进行优化，得到每一帧的位姿
    map<double, ImageFrame>::iterator frame_it;
    ...
    for (int i = 0; frame_it != all_image_frame.end(); frame_it++)
    {
        ...
        // 非关键帧，赋值附近关键帧的位姿，然后利用已知位置的3D点，PnP求解出其位姿
        ...
        // 这里也同样需要将坐标变换矩阵转变为图像帧位姿，并转换为IMU坐标系的旋转
    ...
    // sfm求解出了相邻图像间的平移、旋转和速度，接下来供给图像IMU对齐使用
    // 进行视觉惯性联合初始化
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (corres.size() > 20)
        {
            ...
            // 判断是否满足初始化条件：视差>30和内点数满足要求（大于15）
...
void Estimator::vector2double()
{
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        ...
        if (USE_IMU)
        {
            ...
            std::cout << "Ba_x: " << Bas[i].x() << std::endl;
            std::cout << "Ba_y: " << Bas[i].y() << std::endl;
            std::cout << "Ba_z: " << Bas[i].z() << std::endl;

            std::cout << "Bg_x: " << Bgs[i].x() << std::endl;
            std::cout << "Bg_y: " << Bgs[i].y() << std::endl;
            std::cout << "Bg_z: " << Bgs[i].z() << std::endl;
        }
    }
    ...
    // getFeatureCount()只包括那些被观测了4次及以上的特征点
...
void Estimator::optimization()
{
    ...
    // 因为ceres用的是double数组，所以下面用vector2double()做类型转换
    vector2double();
    ...
    // 边缘化处理
    // 如果次新帧是关键帧，将边缘化最老帧，以及看到的路标点和IMU数据，将其转化为先验
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        // 6.调整参数块在下一次窗口中对应的位置(往前移一格)，注意这里是指针，后面slideWindow中会有赋新值，这里只是提前占座
