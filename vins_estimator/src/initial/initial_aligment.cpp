...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d *Bgs)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        ...
        VectorXd tmp_b(3);
        tmp_b.setZero();
        // q_ij是通过视觉SFM得到的两帧旋转之差，c0代表第L帧，也就是初始化时选出的滑窗基准帧
        // q_ij = q_c0_bi.inverse() * q_c0_bj
        Eigen::Quaterniond q_ij(frame_i->second.R.transpose() * frame_j->second.R);
        // frame_j->second.pre_integration是IMU预积分得到的两帧旋转之差
...
// 施密特正交化和向量叉乘得到重力矢量在半球面的切面的一对正交基b和c
...
void RefineGravity(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    // 由于重力模长固定已知，所以重力待优化变量的维度为2，也就是w1和w2
...
{
    int all_frame_count = all_image_frame.size();
    // 每帧速度：all_frame_count * 3，重力：3，尺度：1
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        ...
        // 除以100是为了避免s过小产生截断误差，将其放大100倍，后面计算出来还要除以100
    ...
    // 检查重力矢量的模长和尺度是否异常，然后进行重力矢量进一步优化
