                                 ...
                                 Vector2d &point0, Vector2d &point1, Vector3d &point_3d)
{
    ...
    // 使用SVD分解的方式进行三角化，对D矩阵进行SVD，V的最后一列即为所求齐次坐标
                          ...
                          const Vector3d relative_T, vector<SFMFeature> &sfm_f, map<int, Vector3d> &sfm_tracked_points)
{
    ...
    // intial two view
    // 将第l帧设置为参考帧（世界坐标系），根据当前帧到第l帧的relative_R、relative_T，得到当前帧位姿
    q[l].w() = 1;
    q[l].x() = 0;
    q[l].y() = 0;
    q[l].z() = 0;
    T[l].setZero();
    // relative_R和relative_T的作用是将当前帧坐标系下的点转换到第l帧坐标系下
    // 也就是可以将第l帧坐标系转换到当前帧坐标系
    ...
    // 设置当前帧的变换矩阵，表示如何将世界坐标系下的点转换到当前帧坐标系下
    ...
    // 2.pnp求解从第l+1开始的每一帧到第l帧的变换矩阵R_initial，P_initial，保存到Pose中
    // 并与当前帧进行三角化
    ...
    */
    // full BA
    // 使用ceres进行全局BA优化，优化图像帧的位姿和特征点的位置
